...
) : CliktCommand(name = "delete", help = "Delete a coffee by id") {
	private val coffeeId: Long by option("-id", help = "Coffee ID").long().prompt()
	override fun run() {
		coffeeService.getCoffee(coffeeId) ?: fail("Coffee not found")
		if (YesNoPrompt("Continue?", terminal, default = false).ask() == true) {
			coffeeService.deleteCoffee(coffeeId)
			terminal.info("Coffee deleted successfully")
		}
	}
}